...
def countRepetition(previous_pose, current_pose, previous_state, flag):
    if current_pose[0][10][0] == 0 and current_pose[0][10][1] == 0:
        ...
        for i in range(5, 17):
            ...
            a = np.array([current_pose[0][5][0], current_pose[0][5][1]])
            b = np.array([current_pose[0][7][0], current_pose[0][7][1]])
            c = ([current_pose[0][9][0], current_pose[0][9][1]])
            ba = a - b
            bc = c - b
            cosine_angle = np.dot(ba, bc) / (np.linalg.norm(ba) * np.linalg.norm(bc))
            angle = np.arccos(cosine_angle)
            angle1 = (f"angle : {np.degrees(angle)}")
        ...
        if(current_state != previous_state):
            flag = (flag + 1)%2
        return angle1, current_pose, current_state.copy(), flag
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            for i, line in enumerate(text.split('\n')):
                y = y0 + i*dy
                image = cv2.putText(image, line, (10, y), cv2.FONT_HERSHEY_SIMPLEX, .5, (0,0,255),3)
            cal_burn= round(count*0.2625 , 2)
            text = (f"count:{count}  cal_burn:{cal_burn}")
            image = cv2.putText(image, text, (950, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255),4)
