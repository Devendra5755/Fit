...
def countRepetition(previous_pose, current_pose, previous_state, flag):
    if current_pose[0][10][0] == 0 and current_pose[0][10][1] == 0:
        ...
        for i in range(5, 17):
            ...
            a= np.array([current_pose[0][7][0] , current_pose[0][7][1]])
            b= np.array([current_pose[0][9][0] , current_pose[0][9][1]])
            c= ([current_pose[0][11][0] , current_pose[0][11][1]])
            ba = a - b
            bc = c - b
            cosine_angle = np.dot(ba, bc) / (np.linalg.norm(ba) * np.linalg.norm(bc))
            angle = str(np.arccos(cosine_angle))
        ...
        if(current_state != previous_state):
            flag = (flag + 1)%2
        return angle, current_pose, current_state.copy(), flag
...
def main():
    with tf.Session() as sess:
        ...
        while True:
            ...
            cal_burn= count*2
            text = (f"count:{count} \n  cal_burn:{cal_burn}")
            image = cv2.putText(image, text, (10, y+20), cv2.FONT_HERSHEY_SIMPLEX, .5, (255,0,0),2)
